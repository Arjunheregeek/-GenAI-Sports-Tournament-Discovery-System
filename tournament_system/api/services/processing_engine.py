...
class TournamentProcessingEngine:
    ...
    def generate_sport_queries(self, sport: str) -> List[Dict[str, Any]]:
        ...
        if not self.query_generator.is_sport_supported(sport):
            supported_sports = self.query_generator.get_supported_sports()
            raise Exception(f"Sport '{sport}' is not supported. Supported sports: {', '.join(supported_sports)}")
        
        # Generate queries specifically for this sport
        sport_queries = self.query_generator.generate_sport_specific_queries(sport)
        
        if not sport_queries:
            raise Exception(f"No queries generated for sport: {sport}")
        
        print(f"✅ Generated {len(sport_queries)} queries for {sport}")
    ...
    def collect_search_results(self, queries: List[Dict[str, Any]], results_per_query: int = 8) -> List[Dict]:
        ...
        print(f"🔍 Processing {len(queries)} queries with {results_per_query} results each...")
    ...
    def extract_tournaments(self, search_results: List[Dict], max_urls: Optional[int] = None) -> List[Dict]:
        ...
        tournaments = self.content_extractor.extract_tournaments_batch(
            search_results, 
            max_urls=max_urls
        )
